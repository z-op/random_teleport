...
minetest.register_privilege("rtp", {
...
minetest.register_chatcommand("rtp", {
    ...
    func = function(name)
        ...
        if not minetest.check_player_privs(name, {rtp = true}) then
            minetest.chat_send_player(name, "Error: You don't have permission to use random teleportation!")
            return false
        end
        ...
        if teleporting == false then
            ...
            minetest.after(3, function()
                ...
                if player_pos then
                    local target_node = minetest.get_node(player_pos)
                    if target_node then
                        ...
                        if node_name then
                            while node_name ~= "air" do
                                ...
                                local pos = vector.new(x, y, z)
                                player:set_pos(pos)
                                node_name = minetest.get_node(player:get_pos()).name
                            ...
                            if failed == true then
                                minetest.chat_send_player(name, "Teleportation failed! Returned to " .. minetest.pos_to_string(player:get_pos()) .. ".")
                            else
                                minetest.chat_send_player(name, "Teleported to " .. minetest.pos_to_string(player:get_pos()) .. ".")
                            end
                        end
                    end
                end
